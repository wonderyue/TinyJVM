...
class Frame:
    ...
    def set_local_long(self, index, val):
        self._local_vars[index:index +
                         2] = struct.unpack("ii", struct.pack("l", val))

    def get_local_long(self, index):
        return struct.unpack("l", struct.pack("ii", self._local_vars[index], self._local_vars[index+1]))[0]

    def set_local_double(self, index, val):
        self._local_vars[index:index +
                         2] = struct.unpack("ii", struct.pack("d", val))

    def get_local_double(self, index):
        return struct.unpack("d", struct.pack("ii", self._local_vars[index], self._local_vars[index+1]))[0]
    ...
    def push_args(self, pre_frame):
        for i in range(self.method.arg_index):
            self.set_local(self.method.arg_index - i -
                           1, pre_frame.pop_operand())
